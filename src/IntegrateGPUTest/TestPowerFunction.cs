...
{
    [TestClass]
    public sealed class TestPowerFunction
    {
        [TestMethod]
        public void TestXCubedFunction()
        {
            // f(x) = x^3, integral from 0 to 2 is 4
            Func<torch.Tensor, torch.Tensor> func = (torch.Tensor x) =>
            {
                return torch.pow(x, 3.0D);
            };
            ...
            double expectedIntegral = 4.0D;
            double tolerance = 0.01D;

            Assert.AreEqual(expectedIntegral, estimatedIntegral, tolerance, "Integral of f(x)=x^3 did not match expected value.");
        }
    }
}