...
{
    [TestClass]
    public sealed class TestSqrtXFunction
    {
        [TestMethod]
        public void TestSqrtX()
        {
            // f(x) = sqrt(x), integral from 0 to 4 is 16/3
            ...
            double expectedIntegral = 16.0D / 3.0D;
            double tolerance = 0.01D;

            Assert.AreEqual(expectedIntegral, estimatedIntegral, tolerance, "Integral of f(x)=sqrt(x) did not match expected value.");
        }
    }
}