...
{
    [TestClass]
    public sealed class TestXToTheFourthFunction
    {
        [TestMethod]
        public void TestXToTheFourth()
        {
            // f(x) = x^4, integral from 0 to 1 is 1/5
            Func<torch.Tensor, torch.Tensor> func = (torch.Tensor x) =>
            {
                return torch.pow(x, 4.0D);
            };
            ...
            double expectedIntegral = 1.0D / 5.0D;
            double tolerance = 0.01D;

            Assert.AreEqual(expectedIntegral, estimatedIntegral, tolerance, "Integral of f(x)=x^4 did not match expected value.");
        }
    }
}