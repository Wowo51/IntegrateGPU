...
{
    [TestClass]
    public sealed class TestTwoDQuadraticSumFunction
    {
        [TestMethod]
        public void TestXPlusYSquaredFunction()
        {
            // f(x,y) = (x+y)^2, integral over [0,1]^2 is 7/6
            Func<torch.Tensor, torch.Tensor> func = (torch.Tensor samples) =>
            {
                ...
                return (x_coords.add(y_coords)).pow(2.0D);
            };
            ...
            Assert.AreEqual(expectedIntegral, estimatedIntegral, tolerance, "Integral of f(x,y)=(x+y)^2 did not match expected value.");
