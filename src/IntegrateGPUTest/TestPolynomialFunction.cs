...
{
    [TestClass]
    public sealed class TestPolynomialFunction
    {
        [TestMethod]
        public void TestTwoXPlusThreeFunction()
        {
            // f(x) = 2x + 3, integral from 0 to 2 is 10
            Func<torch.Tensor, torch.Tensor> func = (torch.Tensor x) =>
            {
                return x.mul(2.0D).add(3.0D);
            };
            ...
            Assert.AreEqual(expectedIntegral, estimatedIntegral, tolerance, "Integral of f(x)=2x+3 did not match expected value.");
        }
    }
}